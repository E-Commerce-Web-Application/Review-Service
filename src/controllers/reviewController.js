...
exports.getReviewsByShopId = async (req, res, next) => {
  try {
    ...
    const reviews = await Review.find({ shopId: shop_id })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
...
/**
 * GET /reviews/shop/:shop_id/average
 */
exports.getAverageRatingByShopId = async (req, res, next) => {
  try {
    const { shop_id } = req.params;
